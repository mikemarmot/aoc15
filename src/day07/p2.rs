...
use super::p1::wirebox::{WireBox, Wire};
...
pub fn doit() {
    ...
    let input2: Vec<String> = input.iter().map(|x| if x.ends_with("-> b") { format!("{} -> b", a) } else { String::from(x) }).collect();
    let res = calc(&input2);
    println!("Result of day07 p2: {:#?}", res.get("a").unwrap());
}

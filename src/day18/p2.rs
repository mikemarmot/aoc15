...
pub fn doit() {
    ...
    println!("Result of day18 p2: {}", res);
}

fn calc(input: &Vec<Vec<bool>>, steps:usize) -> usize {
    let (minx, maxx) = (0, input[0].len() - 1);
    let (miny, maxy) = (0, input.len() - 1);
    ...
    for _i in 1..=steps {
        for (y, v) in input.iter().enumerate() {
            for (x, _e) in v.iter().enumerate() {
                new[y][x] = match (x,y) {
                    (x, y) if x == maxx && y == maxy => true,
                    (x, y) if x == maxx && y == miny => true,
                    (x, y) if x == minx && y == maxy => true,
                    (x, y) if x == minx && y == miny => true,
                    (x, y) => val(&old, x, y),
                };
                //new[y][x] = val(&old, x, y);
            }
...
mod tests {
    ...
    fn test_calc() {
        ...
        assert_eq!(super::calc(&data, 5), 17);
    }
}